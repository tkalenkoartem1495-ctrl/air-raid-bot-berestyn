...
- Знак "+" або "дали" або "є" = ВІДНОВЛЕННЯ (послугу дали, світло/вода з'явилось).
Приклади скарг на відключення: "Піщанка -", "Высокое -", "Центр нема", "Мікрорайон відключили"
Приклади відновлення: "Піщанка +", "Высокое+", "Центр дали", "Є!"
...
4. Якщо люди СТВЕРДЖУЮТЬ про відсутність (знак "-", "нема", "відключили", "зникло") — це ФАКТИЧНЕ відключення. Використовуй 🔴 (світло) або 🔵 (вода): "Відключення...".
5. Якщо люди СТВЕРДЖУЮТЬ про відновлення (знак "+", "дали", "є", "з'явилось") — це ВІДНОВЛЕННЯ. Використовуй 🟢: "Відновлення...".
...
[LIGHT] 🟢 Відновлення світла: мікрорайон Високе, Піщанка (мешканці повідомляють про відновлення світла).
...
[WATER] 🟢 Відновлення води: мікрорайон Центральний.
"""
...
class UtilityMonitor:
    ...
    def _format_status_message(self, clean_text: str) -> str:
        ...
        match = re.match(r"(🔴 Відключення світла:|🔵 Відключення води:|🟡 Питання щодо наявності світла:|🟡 Питання щодо наявності води:|🟢 Відновлення світла:|🟢 Відновлення води:)\s*(.*)", clean_text)
        if match:
    ...
    async def _process_batch_loop(self):
        ...
        while True:
            ...
            dynamic_prompt += "- Якщо є повідомлення про відключення, відновлення або питання про світло, створи ОДНЕ зведене повідомлення і почни його з тегу [LIGHT].\n"
            dynamic_prompt += "- Якщо є повідомлення про відключення, відновлення або питання про воду, створи ОДНЕ зведене повідомлення і почни його з тегу [WATER].\n"
